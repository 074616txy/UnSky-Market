...
- 增加重复注册校验，避免手机号或账号重复
  相关补充详见：[[项目日记-day03🌿-用户体系 — 用户认证与安全补强#二、重复注册校验机制补充|重复注册校验机制补充]]
- 对密码进行加密存储，提升安全性
  相关补充详见：[[项目日记-day03🌿-用户体系 — 用户认证与安全补强#三、密码加密逻辑完善|密码加密逻辑完善]]
- 补充参数合法性校验
- 细化异常提示与返回信息
...
- 登录成功后返回 JWT Token
  相关补充详见：[[项目日记-day03🌿-用户体系 — 用户认证与安全补强#三、密码加密逻辑完善|密码加密逻辑完善]]
- 使用加密后的密码进行比对 
  相关补充详见：[[项目日记-day03🌿-用户体系 — 用户认证与安全补强#四、JWT 认证链路补强|JWT 认证链路补强]]
- 细化登录失败场景的返回提示
- 为后续的接口鉴权、拦截器校验做好准备
...
- [x] 先补一个 `/api/user/info` 受保护接口，正式验证“登录成功 → 携带 Token → 查询当前用户信息”这条链路是否真正打通
  - 在 `UserController` 中声明 `/api/user/info`
  - 在 `UserService`、`UserServiceImpl` 中补充根据 `userId` 查询当前用户信息的方法
  - 在 `JwtUtil` 中继续补充 Token 解析能力，将 Token 中的 `userId` 取出
  - 在 Controller 中通过请求头接收 Token，并替换临时测试写法，正式根据真实登录用户身份查询数据
  - 继续使用 Apifox 分两轮测试：
    1. 先用临时固定 `1L` 跑通基础查询链路
    2. 再携带登录接口返回的 Token 验证真实受保护接口访问

- [x] 在 `/api/user/info` 链路跑通之后，继续进入 day04 的学生身份认证主线
  - 在 MySQL 中新建 `student_cert` 表，保存学生认证申请记录
  - 创建 `StudentCert` 实体类以及对应的 Mapper / Service / Controller 骨架
  - 实现提交认证申请接口：登录用户填写学校、学号、证件照片等信息后提交认证资料
  - 实现查询认证状态接口：用户能够查看自己的认证审核进度
  - 实现管理员审核接口：对认证申请进行通过 / 拒绝处理，并同步更新用户认证状态

- [x] 为后续商品发布权限控制埋下前置条件
  - 逐步将“是否认证通过”与后续业务能力绑定
  - 为之后“未认证用户不能发布商品”这一规则做好状态字段与逻辑准备
