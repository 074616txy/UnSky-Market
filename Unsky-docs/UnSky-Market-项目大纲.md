...
**当前处理：**  
本阶段暂不单独实现真实图片上传接口，商品图片先通过 `images` 字段保存图片路径或静态资源地址，用于支撑商品列表和详情页展示。

**暂缓原因：**  
图片上传涉及文件存储、文件名去重、文件类型校验、访问路径映射、前端上传组件以及后续可能的云存储服务。当前项目优先目标是打通校园二手交易平台的核心业务链路，因此图片上传模块暂缓处理。
...
### Day 09  信用体系 — 交易评价 & 信用管理 ✓ 已完成

🎯**目标：** 交易完成后能互相评价，信用分动态调整

 **记录方式说明：Day 09 采用单篇记录结构。**
> - 主篇：[[项目日记-day09-信用体系 — 交易评价 & 信用管理]]
>
> 本阶段完成了交易核心链路后的**评价与信用体系模块**：围绕订单完成后的买卖双方评价场景，完成评价表设计、评价发布、收到评价查询、发出评价查询、匿名评价展示、评分文本转换等核心能力；同时设计了基于评分的信用分动态调整规则，使用 `5 分 +2、4 分 +1、3 分不变、2 分 -1、1 分 -2` 管理用户信用变化，并通过订单参与者校验、订单完成状态校验、同一订单同一用户不可重复评价等规则保证评价数据真实有效，为后续 Day10 管理员后台与平台风控管理提供信用数据基础。

**前置依赖：** Day 08（需要订单完成后才能评价）

**操作清单：**

- [x] 在 MySQL 中新建 `review` 表（id、order_id、from_user_id、to_user_id、score、content、is_anonymous、create_time）
- [x] 创建 `Review.java`、`ReviewVO.java`
- [x] 实现评价接口 `/api/review/add`（订单状态为已完成才能评价）
- [x] 实现查询评价接口（查看某用户的全部评价 / 某商品的评价）
- [x] 实现信用分计算逻辑：每次评价后更新被评价方的 `creditScore`
- [x] 在用户详情接口中返回信用评分和评价统计
- [x] 验证：未完成订单不能评价，同一订单不能重复评价

**交付物：** 评价接口上线，信用分动态更新，评价展示正确

---

### Day 10  后台管理 — 管理员模块 ✓ 已完成
...
### Day 11  项目收尾 — 部署规划 & 项目包装 ✓ 已完成

🎯**目标：** 项目打包部署到云服务器，能通过域名访问

**记录方式说明：Day 11 采用单篇记录结构。**
> - 主篇：[[项目日记-day11-项目收尾 — 部署规划 & 项目包装]]
>
   >本阶段完成了 UnSky Market 从本地开发环境到公网访问环境的部署实践。项目通过 Docker 分别运行 MySQL、Redis、Spring Boot 后端和 Nginx 前端容器，并由 Nginx 托管前端 `dist` 页面，同时将 `/api` 请求反向代理到后端服务。最终项目成功部署到阿里云 ECS，能够通过公网 IP 访问，完成了从“本地项目”到“公网可展示项目”的关键一步。

**前置依赖：** 前 10 天所有功能基本跑通

**操作清单：**

- [x] 在 `application.yml` 中配置生产环境数据库连接
- [x] 修改 `pom.xml` 的打包配置（指定最终 jar 包名）
- [x] 执行 `mvn clean package -DskipTests`，验证 jar 包生成
- [x] 购买云服务器（或使用学生机），安装 JDK 11 + MySQL 8 + Redis
- [x] 将 jar 包上传至服务器，用 `nohup java -jar xxx.jar &` 后台运行
- [x] 配置 Nginx 反向代理（80 端口 → 8080）
- [x] 申请域名并配置 DNS 解析
- [x] 申请 SSL 证书，配置 HTTPS
- [x] 用 knife4j 或 Swagger 生成在线接口文档
- [x] GitHub 仓库整理（代码 + README.md + 项目文档）
- [x] 验证：通过域名访问接口，全部功能正常
