...
- [ ] 将拦截器注册到 `WebMvcConfigurer`，放行登录/注册接口
- [ ] 实现个人中心接口 `/api/user/info`（需带 Token，返回用户基本信息）
- [ ] 实现修改个人信息接口（昵称、头像、学校）
- [ ] 实现退出登录接口（服务端 Token 黑名单 or 前端删除 Token）
- [ ] 用 Postman 测试所有接口，验证注册→登录→访问受保护接口的完整链路

**交付物：** 用户注册/登录/信息修改/Token 鉴权全流程跑通
...
| Day 03 | BCrypt 密码加密、JWT Token 颁发与验证、拦截器 |
| Day 04 | 文件上传基础、表单校验 |
