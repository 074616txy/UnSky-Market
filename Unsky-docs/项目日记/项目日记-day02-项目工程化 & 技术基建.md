...
>   1. 确定后续开发继续沿用 `com/Market/` 下按业务分模块扩展的方式
>   2. 在`user`模块下明确`controller / mapper / service / service/impl`四层职责
>   3. 搭出UserController，UserMapper、UserService，UserServiceImpl基础骨架
>   4. 实现注册接口 `/api/user/register`
>   5. 参数：手机号/用户名 + 密码
...
>  13. 新增登录拦截器与配置类，放行注册/登录接口，保护需要登录才能访问的接口
>  14. 使用Apifox按完成测试：注册成功 → 登录成功 → 拿 Token → 访问受保护接口
### 这一阶段最重要的理解
...
## 八，我想说：٩(˃̶͈̀௰˂̶͈́)!!! (2026/4/22)
 ...
 zzz…(～﹃～)~zZ  累累累 ！！！
   另外我也修改项目大纲，再分模块方面我把框架做了最终的定型，我觉得这很重要，所以最终结果也是我深思熟虑的结果！٩(˃̶͈̀௰˂̶͈́)و最后，都说做成一件事都是从0到1最难，我也是深深的体会到了，希望后面的内容善待我吧  (｡•́︿•̀｡)，☁️ 🌊 ⛵收工收工，疲惫手工(˃̵ᴗ˂̵)و下面尽情期待后面day03的内容吧！】
